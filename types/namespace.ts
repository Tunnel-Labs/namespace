...
export type OverridePropertyKeys<
	$Properties,
	$OverrideSuffix extends string,
> = keyof $Properties & `${string}${$OverrideSuffix}`;

// dprint-ignore
export type PropertiesWithOverride<$Properties, $OverrideSuffix extends string> = Omit<
  $Properties,
...
> = {
	[
		K in keyof Properties as K extends
			`${Prefix}${Separator}${infer MethodName}` ? MethodName :
			K
	]: Properties[K];
};
